            ...
            if c == 'box':
                ...
                for line in rd.readlines():
                    ...
                    if (len(value) == 0):
                        continue
                    if value[0] == 'v':
                        v.append([100*float(value[1]), 100*float(value[2]), 100*float(value[3])])
                    if value[0] == 'vn':
                        n.append([100*float(value[1]), 100*float(value[2]), 100*float(value[3])])
                    if value[0] == 'f':
                        face = []
                        for j in range(1, len(value)):
                            if value[j].count('/') == 0:
                                ...
                                s = value[j].split('/')
                                f.append(v[int(s[0])-1])
                        if len(face) > 2:
                            for i in range(2, len(face)):
                                add_polygon(polygon, face[0][0],face[0][1],face[0][2],
                                                 face[i-1][0],face[i-1][1],face[i-1][2],
                                                 face[i][0],face[i][1],face[i][2])
                                add_polygon(polygon, face[i][0],face[i][1],face[i][2],
                                                 face[i-1][0],face[i-1][1],face[i-1][2],
                                                 face[0][0],face[0][1],face[0][2])

                matrix_mult( stack[-1], polygon)
                draw_polygons(polygon, screen, zbuffer, view, ambient, light, symbols, reflect)
                rd.close()
            elif c == 'sphere':
                if command['constants']:
                    reflect = command['constants']
